...
function DataTable<T extends object>({
  ...
  // 2. Sort logic (performed on filtered data)
    ...
    <div style={{ fontFamily: 'sans-serif' }}>
      ...
      {searchableKey && (
        <input
          ...
          style={{ marginBottom: 12, padding: '8px', width: '100%', boxSizing: 'border-box' }}
        />
      )}
          ...
          <tr style={{ background: '#1e3a8a', color: '#fff' }}>
            {columns.map(col => (
              ...
              >
                {col.header}
                {col.sortable && (
                  <span style={{ marginLeft: 8, opacity: sort.key === col.key ? 1 : 0.3 }}>
